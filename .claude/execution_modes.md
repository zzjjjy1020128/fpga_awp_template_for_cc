...
### 必须依赖的文件
- `.claude/orchestration_guide.md` —— 编排策略与合规分层
...
### FPGA 项目特别注意
- rtl_implementer、integration_verifier、vivado_integrator 可并行工作
...
每次新 session 启动时，在恢复上下文之前：

1. 运行 `python -c "import yaml"` 检查 PyYAML 是否安装
2. 如果 import 失败：运行 `pip install -r requirements.txt`
...
每次新 session 启动时，在执行任何其他工作之前：

1. `.awp/handoffs/` 中检查是否存在未读的 handoff 文件（按日期排序，取最新的）
2. 若存在 handoff：读取内容，恢复上一 session 的上下文（已完成/未完成 task、关键文件、已知问题），从"下一步行动"开始继续
3. 若不存在 handoff：检查 `.awp/task_board.md` 确认当前项目状态
4. 向用户汇报恢复结果

**平台检查**：读取 `workspace_manifest.json` → `platforms[]`，若存在已冻结平台则加载对应 `.awp/platform/hw_base_*.yaml`，打印可用平台摘要（器件、频率、验证状态）。后续 task 派发时优先匹配 task 的 `target_platform` 到对应平台。

**Handoff 恢复后的 gate re-validation（强制）**：
...
- [ ] **Scope 兼容检查**：检查 task 合同的 `forbidden_edit_paths` 是否与当前 agent 定义的能力匹配
...
|------|-------|------|:--:|
| **模板** | `awp` | `.awp/templates/`、`.awp/schemas/`、`.awp/registry/` | 是 |
| | `conf` | `CLAUDE.md`、`LAYERS.md`、`Makefile`、`requirements.txt`、`.gitmessage`、`.gitignore`、`.claude/`、`scripts/` | 是 |
| **项目** | `rtl` | `rtl/` | 否 |
| | `tb` | `tb/` `sim/` | 否 |
...
| | `docs` | `docs/` | 否 |
| | `session` | `.awp/tasks/`、`.awp/sessions/`、`.awp/handoffs/`、`.awp/reviews/`、`.awp/retrospectives/` `.awp/task_board.md`、`.awp/decisions.md`、`.awp/registry/id_registry.yaml`、`.awp/registry/relations.yaml` | 否 |
